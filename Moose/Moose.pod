...
Moose 會將一些函式匯出到類別的名稱空間中，以用來幫助設定此類別。（由於這些函數是在此類別的名稱空間中）所以這些函數可以在這個類別中直接使用（這段我不太知道該如何翻的通順...）
...
每個父類別會包含一組雜湊選項，不過目前只有支援 L<-version|Class::MOP/Class Loading Options> 。
